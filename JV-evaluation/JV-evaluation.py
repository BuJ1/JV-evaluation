...
class main_window(QtWidgets.QMainWindow):
    ...
    def initUI(self):
        ...
        self.saveAction = QtWidgets.QAction(
            QtGui.QIcon('icons/save_as.png'), 'Save', self)
        self.changeFilePathAction = QtWidgets.QAction(QtGui.QIcon(
            'icons/select_folder.png'), 'Change File Path', self)
        self.UVVisFittingAction = QtWidgets.QAction(
            QtGui.QIcon('icons/fit_graph.png'), 'Fit Onset', self)
        self.normalizeUVVisAction = QtWidgets.QAction(QtGui.QIcon(
            'icons/normalize_UVVis.png'), 'Normalize UV-Vis Spectra', self)
        self.assignGroupAction = QtWidgets.QAction(
            QtGui.QIcon('icons/assign_group.png'), 'Assign Group', self)
        self.showGroupAction = QtWidgets.QAction(QtGui.QIcon(
            'icons/show_graphs.png'), 'Show Group of IVs', self)
        self.extractMobilityAction = QtWidgets.QAction(
            QtGui.QIcon('icons/calculator.png'), 'Calculate Mobility', self)
        self.showOverviewAction = QtWidgets.QAction(QtGui.QIcon(
            'icons/show_overview.png'), 'Show Overview over Performance Parameters', self)
        self.showHeroDevicesAction = QtWidgets.QAction(
            QtGui.QIcon('icons/hero_dev.png'), 'Show Hero Devices', self)
        self.updateLegendAction = QtWidgets.QAction(QtGui.QIcon(
            'icons/update_legend2.png'), 'Update the Legend', self)
        self.transposeXAction = QtWidgets.QAction(
            QtGui.QIcon('icons/transposeX.png'), 'Fit Onset', self)
        self.TPVAction = QtWidgets.QAction(
            QtGui.QIcon('icons/TPV.png'), 'Plot TPV', self)
        self.TPCAction = QtWidgets.QAction(
            QtGui.QIcon('icons/TPC.png'), 'Plot TPC', self)
        self.intDepIVAction = QtWidgets.QAction(QtGui.QIcon(
            'icons/intensity_dep.png'), 'Plot Intensity Dependent IV', self)
        ...
        self.statusBar().showMessage(
            "Welcome: Select a Folder from which Data shall be read", 30000)
        ...
        self.mobilityMeasurementAction.triggered.connect(
            self.mobilityMeasurement)
        ...
        self.showHeroDevicesAction.triggered.connect(
            functools.partial(self.showHeroDevices, "group"))
    ...
    def readData(self, files, names_, plotVars_, skipr_=[]):
        ...
        try:
            ...
            warnings.warn(
                "Could not load example data to figure out the file size.", category=UserWarning)
            return
        ...
        try:
            ...
            warnings.warn(
                "Could not figure out how many rows should be skipped", category=UserWarning)
        ...
        for filename in files:
            try:
                ...
                # python
                graphLabels[int(
                    idx / 2)] = os.path.splitext(os.path.basename(filename))[0]
    ...
    def plot(self, data, labels_, xylabel_, tickInterval_,
            ...
            if errbar_ == False:
                try:
                    self.line["line" + str(dataNb)], = ax_.plot(data[self.idx], data[self.idx+1],
                                                                linewidth=self.linew, label=labels_[
                                                                    dataNb],
                    ...
                    self.line["line" + str(dataNb)], = ax_.plot(data[self.idx], data[self.idx+1],
                                                                linewidth=self.linew, label=labels_[
                                                                    dataNb], marker=marker_,
                                                                linestyle=linestyle_)
            else:
                try:
                    self.line["line" + str(dataNb)] = ax_.errorbar(data[self.idx],
                                                                   data[self.idx +
                                                                        1], yerr=data[self.idx + 2], fmt=marker,
                                                                   capsize=self.linew, label=labels_[
                                                                       dataNb],
        ...
        self.lines = [self.line["line" + str(i)]
                      for i in range(int(len(data) / fileSz))]
        self.lined = dict()
        self.lineLabel = dict()
        ...
        loc = plticker.MultipleLocator(base=tickInterval_)
        ...
        if minorTicks_ == True:
            ...
            ax_.tick_params(which="minor", direction="in", bottom=True, top=True,
                            length=self.tick_length/4*3, width=self.ax_ticks_width)
        # ax__top.xaxis.set_minor_locator(minorLocator)

        ax_.tick_params(width=self.ax_ticks_width, length=self.tick_length)
        ax_.tick_params(which="major", direction="in",
                        bottom=True, top=True, width=self.ax_ticks_width)
        ax_.tick_params(which="major", direction="in", left=True,
                        right=True, width=self.ax_ticks_width)
        ...
        if errbar_ == False:
            leg = ax_.legend(
                frameon=False, fontsize=self.fonts_, loc=self.legend_pos)
        ...
        def override(**kwargs):
            ...
            labelVoc = "Voc: " + str(np.round(self.performance_data[np.where(
                kwargs['label'] == self.IVlabels), 1][0][0], decimals=2))
            labelJsc = "Jsc: " + str(np.round(self.performance_data[np.where(
                kwargs['label'] == self.IVlabels), 2][0][0], decimals=2))
            labelFF = "FF: " + str(np.round(self.performance_data[np.where(
                kwargs['label'] == self.IVlabels), 3][0][0], decimals=2))
            labelPCE = "PCE: " + str(np.round(self.performance_data[np.where(
                kwargs['label'] == self.IVlabels), 4][0][0], decimals=2))
    ...
    def updateLegend(self):
        ...
        for leg_elem in range(len(self._ax.get_legend_handles_labels()[0])):
            if self._ax.get_legend_handles_labels()[0][leg_elem].get_visible():
                handles_new_leg = np.append(
                    handles_new_leg, self._ax.get_legend_handles_labels()[0][leg_elem])
                labels_new_leg = np.append(
                    labels_new_leg, self._ax.get_legend_handles_labels()[1][leg_elem])
        ...
        for k in range(len(self._ax.get_legend_handles_labels()[0])):
            ...
            if self._ax.get_legend_handles_labels()[1][k] in self.GroupNames.astype('U13'):
                elemNb = np.where(self.GroupNames.astype(
                    'U13') == self._ax.get_legend_handles_labels()[1][k])[0][0]
                print(elemNb)
                print(self.GroupColor.astype('U7'))
                h = self.GroupColor.astype('U7')[elemNb].lstrip('#')
                self.line["line" + str(k)].set_color(
                    tuple(int(h[i:i+2], 16) / 255 for i in (0, 2, 4)))

        fonts = 18

        # self._ax.set_legend_handles() = handles_new_leg
        # self._ax.set_legend_handles_labels() = labels_new_leg
        leg = self._ax.legend(handles_new_leg, labels_new_leg,
                              frameon=False, fontsize=fonts)
        ...
        if event.mouseevent.button == 1:
            ...
            try:
                if self.isWhat == "IV":
                    ...
                    extracted_nbs = [int(x) for x in regex.findall(
                        str(self.lineLabel[legline]))]
                    extracted_nbs = np.array(
                        [extracted_nbs[1], extracted_nbs[2], extracted_nbs[3]])
                ...
                if self.isWhat == "mobility":
                    ...
                    if self.devNbisNb:
                        ...
                        extracted_nbs_temp = [str(x) for x in regex.findall(
                            re.search('p(.*)', str(self.lineLabel[legline].get_label()))[0])]
                        s = str(self.lineLabel[legline].get_label())
                        result = re.search(
                            'd(.*)p' + str(extracted_nbs_temp[0]), s)[1]
                        extracted_nbs = np.array(
                            [str(result), extracted_nbs_temp[0], extracted_nbs_temp[1]])
                    ...
                    if temp == None:
                        warnings.warn(
                            "Error the graph could not be traced back", category=UserWarning)
                    if np.size(temp) > 1:
                        print(temp)
                        warnings.warn(
                            "Error more than one graph was selected", category=UserWarning)

            except:
                warnings.warn(
                    "Error happened trying to extract nbs of legend when picking a graph.", category=UserWarning)
            ...
            if vis:
                ...
                try:
                    ...
                    warnings.warn(
                        "Though the IV window is selected, a graph with a label other than its device, pixel and scan number was picked.", category=UserWarning)
            else:
            ...
            if self.isWhat == "IV":
                ...
                dataToPlot = np.array([self.IVdata[2 * temp][:], self.IVdata[2 * temp + 1]
                                       [:], self.DIVdata[2 * temp][:], self.DIVdata[2 * temp + 1][:]])
            ...
            if self.isWhat == "mobility":
                ...
                try:
                    ...
                    dataToPlot = np.array([self.DIVdata[2 * temp][:], self.DIVdata[2 * temp + 1][:],
                                           self.DIVdata[2 * temp][np.where(self.DIVdata[2 * temp] == self.x_min[temp])[
                                               0][0]:np.where(self.DIVdata[2 * temp] == self.x_max[temp])[0][0]],
                    ...
                    warnings.warn(
                        "There is no quadratic regime in this graph it was thus not fitted", category=UserWarning)
    ...
    def changeFilePath(self):
        ...
        if np.size(self.paths) == 1:
            ...
            for i in range(np.size(self.paths)):
                ...
                if os.path.isfile(file_name) == True:
                    ...
                    try:
                        ...
                        GroupNamesGlobalAll = np.append(
                            GroupNamesGlobalAll, np.array(f["GroupNamesGlobal"]))
                        GroupColorAll = np.append(
                            GroupColorAll, np.array(f["GroupColor"]))
                        ...
                        Voc_dev_all = np.append(
                            Voc_dev_all, np.array(f["Voc_dev"]))
                        ...
                        Voc_dev_rev_all = np.append(
                            Voc_dev_rev_all, f["Voc_dev_rev"])
                        Jsc_dev_rev_all = np.append(
                            Jsc_dev_rev_all, f["Jsc_dev_rev"])
                        FF_dev_rev_all = np.append(
                            FF_dev_rev_all, f["FF_dev_rev"])
                        PCE_dev_rev_all = np.append(
                            PCE_dev_rev_all, f["PCE_dev_rev"])
                        print("File of folder " +
                              str(self.paths[i]) + " successfully loaded")
                    except:
                        try:
                            GroupNamesGlobalAll = np.append(
                                GroupNamesGlobalAll, np.array(f["GroupNamesGlobal"]))
                            ...
                            PCE_dev_all = np.append(PCE_dev_all, f["PCE_dev"])
                            print(
                                "File of folder " + str(self.paths[i]) + " successfully loaded. Has no reverse scan.")
                        except:
            ...
            self.GroupNames = np.unique(GroupNamesGlobalAll)[np.unique(
                GroupNamesGlobalAll).astype('U13') != "None"]
            self.GroupNamesGlobal = np.unique(GroupNamesGlobalAll)[
                np.unique(GroupNamesGlobalAll).astype('U13') != "None"]
            ...
            for unGrName in np.unique(GroupNamesGlobalAll.astype('U13')):
                ...
                self.Voc_all.append(np.concatenate(Voc_dev_all[np.where(
                    GroupNamesGlobalAll.astype('U13') == unGrName)], axis=None))
                self.Jsc_all.append(np.concatenate(Jsc_dev_all[np.where(
                    GroupNamesGlobalAll.astype('U13') == unGrName)], axis=None))
                self.FF_all.append(np.concatenate(FF_dev_all[np.where(
                    GroupNamesGlobalAll.astype('U13') == unGrName)], axis=None))
                self.PCE_all.append(np.concatenate(PCE_dev_all[np.where(
                    GroupNamesGlobalAll.astype('U13') == unGrName)], axis=None))

                if np.size(Voc_dev_rev_all) != 0:
                    self.Voc_rev_all.append(np.concatenate(Voc_dev_rev_all[np.where(
                        GroupNamesGlobalAll.astype('U13') == unGrName)], axis=None))
                    self.Jsc_rev_all.append(np.concatenate(Jsc_dev_rev_all[np.where(
                        GroupNamesGlobalAll.astype('U13') == unGrName)], axis=None))
                    self.FF_rev_all.append(np.concatenate(FF_dev_rev_all[np.where(
                        GroupNamesGlobalAll.astype('U13') == unGrName)], axis=None))
                    self.PCE_rev_all.append(np.concatenate(PCE_dev_rev_all[np.where(
                        GroupNamesGlobalAll.astype('U13') == unGrName)], axis=None))
    ...
    def saveFile(self):
        ...
        if self.isDual == True:
            Voc_dev, Jsc_dev, FF_dev, PCE_dev, Voc_dev_rev, Jsc_dev_rev, FF_dev_rev, PCE_dev_rev, labels_ = self.sortParams(
                "groups")
            ...
            Voc_dev, Jsc_dev, FF_dev, PCE_dev, labels_ = self.sortParams(
                "groups")
        ...
        # set_trace()
        f.create_dataset(
            "data_set_path", data=self.paths[0], dtype=h5py.special_dtype(vlen=bytes))
        f.create_dataset("scan_nb", data=self.scan_nb)
        f.create_dataset("IVselected", data=self.IVselected)
        f.create_dataset("GroupNames", data=self.GroupNames,
                         dtype=h5py.special_dtype(vlen=bytes))
        f.create_dataset("GroupNamesGlobal", data=self.GroupNamesGlobal,
                         dtype=h5py.special_dtype(vlen=bytes))
        f.create_dataset("GroupNbs", data=self.GroupNbs,
                         dtype=h5py.special_dtype(vlen=np.dtype('int32')))
        f.create_dataset("GroupColor", data=self.GroupColor,
                         dtype=h5py.special_dtype(vlen=str))
        f.create_dataset("Voc_dev", data=Voc_dev,
                         dtype=h5py.special_dtype(vlen=np.dtype('float64')))
        f.create_dataset("Jsc_dev", data=Jsc_dev,
                         dtype=h5py.special_dtype(vlen=np.dtype('float64')))
        f.create_dataset("FF_dev", data=FF_dev,
                         dtype=h5py.special_dtype(vlen=np.dtype('float64')))
        f.create_dataset("PCE_dev", data=PCE_dev,
                         dtype=h5py.special_dtype(vlen=np.dtype('float64')))

        if self.isDual == True:
            f.create_dataset("Voc_dev_rev", data=Voc_dev_rev,
                             dtype=h5py.special_dtype(vlen=np.dtype('float64')))
            f.create_dataset("Jsc_dev_rev", data=Jsc_dev_rev,
                             dtype=h5py.special_dtype(vlen=np.dtype('float64')))
            f.create_dataset("FF_dev_rev", data=FF_dev_rev,
                             dtype=h5py.special_dtype(vlen=np.dtype('float64')))
            f.create_dataset("PCE_dev_rev", data=PCE_dev_rev,
                             dtype=h5py.special_dtype(vlen=np.dtype('float64')))
        f.close()
        ...
        if self.IVcalled == True:
            ...
            try:
                ...
                try:
                    ...
                    names_ = ["Data File Name", "Voc",
                              "Jsc", "FF", "PCE", "Vmpp", "Impp"]
                    file_data_list = pd.read_csv(filename, skiprows=1, skipfooter=0,
                                                 sep="\t", names=names_, engine="python")
                    file_data_array = np.array(file_data_list)
                except:
                    warnings.warn(
                        "Error while reading in PV parameters Dual. Please Check File for correctness.", category=UserWarning)
                    return
            ...
            performance_data_temp = np.empty(
                (0, np.size(names_)), dtype="object")
            self.performance_data = np.empty(
                (0, np.size(names_)), dtype="object")
            ...
            for nb_files in range(len(files)):
                performance_data_temp = np.append(performance_data_temp,
                                                  file_data_array[np.where(files[nb_files][5:].rsplit('/', 1)[-1] ==
                                                                           file_data_list["Data File Name"])[0]], axis=0)
                # Extract pixel
                regex = re.compile(r'\d+')
                # extracted_nbs = [int(x) for x in regex.findall(files_temp[nb_files].rsplit('/', 1)[-1])]
                # pixel_nb = extracted_nbs[2]
                pixel_nb = [int(x) for x in regex.findall(
                    files[nb_files].rsplit('/', 1)[-1].rsplit('_', 5)[3])][0]
                try:
                    dev_nb = int(files[nb_files].rsplit(
                        '/', 1)[-1].rsplit('_', 4)[1][1:])
                except:
                    dev_nb = files[nb_files].rsplit(
                        '/', 1)[-1].rsplit('_', 4)[1][1:]
                nbs_temp = np.append(
                    nbs_temp, [[dev_nb, pixel_nb, self.scan_nb]], axis=0)
            ...
            for nb_files in range(len(filesDIV)):
                ...
                regex = re.compile(r'\d+')
                pixel_nbDIV = [int(x) for x in
                               regex.findall(filesDIV[nb_files].rsplit('/',
                                                                       1)[-1].rsplit('_', 5)[3])][0]
                try:
                    dev_nb = int(filesDIV[nb_files].rsplit(
                        '/', 1)[-1].rsplit('_', 4)[1][1:])
                except:
                    dev_nb = filesDIV[nb_files].rsplit(
                        '/', 1)[-1].rsplit('_', 4)[1][1:]
                nbsDIV_temp = np.append(
                    nbsDIV_temp, [[dev_nb, pixel_nbDIV, scan_nbDIV]], axis=0)
    ...
    def assignGroup_dialog(self):
        ...
        self.gridLayoutAGD.addWidget(QtWidgets.QLabel(
            "Enter Label Name (Global Name in Parenthesis)"), 1, 0)
        self.gridLayoutAGD.addWidget(QtWidgets.QLabel(
            "Enter Devices (seperated by ,)"), 1, 1)

        self.GroupNamesLineEdit = np.empty(0, dtype="object")
        self.GroupNamesLineEdit = np.append(
            self.GroupNamesLineEdit, QtWidgets.QLineEdit())
        self.gridLayoutAGD.addWidget(self.GroupNamesLineEdit[0], 2, 0)

        if np.size(self.paths) == 1:
            self.GroupNbsLineEdit = np.empty(0, dtype="object")
            self.GroupNbsLineEdit = np.append(
                self.GroupNbsLineEdit, QtWidgets.QLineEdit())
            self.gridLayoutAGD.addWidget(self.GroupNbsLineEdit[0], 2, 1)

        self.GroupColorsButton = np.empty(0, dtype="object")
        self.GroupColorsButton = np.append(
            self.GroupColorsButton, QtWidgets.QPushButton(""))
        self.GroupColorsButton[0].clicked.connect(
            functools.partial(self.changeColor, 0))
        self.GroupColorsButton[0].setStyleSheet(
            "background-color: " + str(self.GroupColor.astype('U7')[0]))
        self.gridLayoutAGD.addWidget(self.GroupColorsButton[0], 2, 2)

        if self.isWhat == "mobility":
            self.gridLayoutAGD.addWidget(QtWidgets.QLabel(
                "Enter Layer Thickness (nm)"), 1, 3)
            self.GroupThickness = np.empty(0, dtype="object")
            self.GroupThickness = np.append(
                self.GroupThickness, QtWidgets.QLineEdit())
        ...
        self.fitButton.clicked.connect(
            functools.partial(self.assignGroup, dialog))

        self.exitButton = QtWidgets.QPushButton("Close")
        self.exitButton.clicked.connect(
            functools.partial(self.close_dialog, dialog))
        ...
        if np.size(self.GroupNames) > 0:
            ...
            for groupNb in range(self._nbGroups):
                try:
                    ...
                    self.GroupNamesLineEdit[groupNb].setText(
                        self.GroupNames.astype('U13')[groupNb])

                if np.size(self.paths) == 1:
                    self.GroupNbsLineEdit[groupNb].setText(
                        ','.join(map(str, self.GroupNbs[groupNb])))
    ...
    def changeColor(self, groupNb):
        ...
        self.GroupColorsButton[groupNb].setStyleSheet(
    ...
    def assignGroup(self, dialog):
        ...
        self.GroupNames = np.empty(
            np.size(self.GroupNamesLineEdit), dtype="object")
        self.GroupNamesGlobal = np.empty(
            np.size(self.GroupNamesLineEdit), dtype="object")

        if np.size(self.paths) == 1:
            self.GroupNbs = np.empty(
                np.size(self.GroupNbsLineEdit), dtype="object")
        self.ThicknessLayer = np.empty(
            np.size(self.ThicknessLayer), dtype="object")
        ...
        for Group in range(np.size(self.GroupNamesLineEdit)):
            ...
            if np.size(self.paths) == 1:
                if self.GroupNbsLineEdit[Group].text() == "":
                    warnings.warn(
                        "Please do enter device/s for your group. Try again!", category=UserWarning)
                    return
                if self.GroupNbsLineEdit[Group].text()[-1] == ",":
                    warnings.warn(
                        "Please do not end your numbers with a comma. Try again!", category=UserWarning)
                    return
                try:
                    mylist = np.array(
                        [int(x) for x in self.GroupNbsLineEdit[Group].text().split(',')])
                except:
                    mylist = np.array(
                        [str(x) for x in self.GroupNbsLineEdit[Group].text().split(',')])
                if self.isWhat == "IV":
                    if np.all(np.in1d(mylist, np.unique(self.nbs[:, 0]))) == False:
                        warnings.warn(
                            "Please only enter valid device numbers.", category=UserWarning)
                        return
                elif self.isWhat == "mobility":
            ...
            if self.GroupNamesLineEdit[Group].text() == "":
                warnings.warn(
                    "Please do enter a name for your group. Try again!", category=UserWarning)
                return
            ...
            if self.isWhat == "mobility":
                self.ThicknessLayer[Group] = float(
                    self.GroupThickness[Group].text())
    ...
    def showGroup_dialog(self):
        ...
        # Define all the layouts and labels so that the window looks good
        verticalLayout = QtWidgets.QVBoxLayout()
        verticalLayout.addWidget(QtWidgets.QLabel(
            "IV Curves of which group or device shall be plotted?"))
    ...
    def showGroup(self, DeviceNumber, darkOrLight):
        ...
        if np.size(DeviceNumber) > 1:
            for devnb in DeviceNumber:
                if darkOrLight == "light":
                    ...
                    tempPixelsOfDeviceIV = np.where(self.nbs[:, 0] == devnb)[0]
                    tempPixelsOfDevice = np.where(
                        self.nbsDIV[:, 0] == devnb)[0]

                PixelsOfDevice = np.append(PixelsOfDevice, tempPixelsOfDevice)
                PixelsOfDeviceIV = np.append(
                    PixelsOfDeviceIV, tempPixelsOfDeviceIV)
        ...
        # If a single device is selected only plot that one
            ...
            if darkOrLight == "light":
                tempPixelsOfDeviceIV = np.where(
                    self.nbs[:, 0] == DeviceNumber)[0]
                tempPixelsOfDevice = np.where(
                    self.nbs[:, 0] == DeviceNumber)[0]
            elif darkOrLight == "dark":
                tempPixelsOfDeviceIV = np.where(
    ...
    def showOverview_dialog(self):
        ...
        if self.multipleFoldersLoaded == False:
            ...
            self.buttonOverviewDevices.clicked.connect(
                functools.partial(self.showOverview, "devices", "forward"))
        ...
        self.buttonOverviewGroup.clicked.connect(
            functools.partial(self.showOverview, "groups", "forward"))
        self.buttonOverviewGroupAvg.clicked.connect(functools.partial(self.showOverview,
                                                                      "groups", "forward", avg=True))
        ...
        if self.isDual == True:
            verticalLayout.addWidget(QtWidgets.QLabel(
                "Show Overview of Reverse Data:"))
            self.horizontalLayoutSODRev = QtWidgets.QHBoxLayout()

            if self.multipleFoldersLoaded == False:
                self.buttonOverviewDevicesRev = QtWidgets.QPushButton(
                    "Devices")
                self.buttonOverviewDevicesRev.clicked.connect(functools.partial(self.showOverview,
                                                                                "devices", "reverse"))
                self.horizontalLayoutSODRev.addWidget(
                    self.buttonOverviewDevicesRev)

            self.buttonOverviewGroupRev = QtWidgets.QPushButton("Group")
            self.buttonOverviewGroupRevAvg = QtWidgets.QPushButton("Avg Group")
            self.buttonOverviewGroupRev.clicked.connect(
                functools.partial(self.showOverview, "groups", "reverse"))
            self.buttonOverviewGroupRevAvg.clicked.connect(functools.partial(self.showOverview,
                                                                             "groups", "reverse", avg=True))

            self.horizontalLayoutSODRev.addWidget(self.buttonOverviewGroupRev)
            self.horizontalLayoutSODRev.addWidget(
                self.buttonOverviewGroupRevAvg)
    ...
    def showOverview(self, groupOrDevice, forwardOrReverse, avg=False):
        ...
        if forwardOrReverse == "forward":
            self.plotOverview(Voc_dev, Jsc_dev, FF_dev, PCE_dev,
                              groupOrDevice, forwardOrReverse, labels_, avg)
        elif forwardOrReverse == "reverse":
            self.plotOverview(Voc_dev_rev, Jsc_dev_rev, FF_dev_rev, PCE_dev_rev, groupOrDevice,
                              forwardOrReverse, labels_, avg)
    ...
    def sortParams(self, groupOrDevice):
        ...
        if groupOrDevice == "devices":
            for i in np.unique(self.nbs[:, 0]):
                # Forward sweep
                Voc_dev.append(self.performance_data[np.logical_and(
                    self.nbs[:, 0] == i, self.IVselected), 1])
                Jsc_dev.append(self.performance_data[np.logical_and(
                    self.nbs[:, 0] == i, self.IVselected), 2])
                FF_dev.append(self.performance_data[np.logical_and(
                    self.nbs[:, 0] == i, self.IVselected), 3])
                PCE_dev.append(self.performance_data[np.logical_and(
                    self.nbs[:, 0] == i, self.IVselected), 4])

                if self.isDual == True:
                    # Reverse sweep
                    Voc_dev_rev.append(self.performance_data[np.logical_and(
                        self.nbs[:, 0] == i, self.IVselected), 7])
                    Jsc_dev_rev.append(self.performance_data[np.logical_and(
                        self.nbs[:, 0] == i, self.IVselected), 8])
                    FF_dev_rev.append(self.performance_data[np.logical_and(
                        self.nbs[:, 0] == i, self.IVselected), 9])
                    PCE_dev_rev.append(self.performance_data[np.logical_and(
                        self.nbs[:, 0] == i, self.IVselected), 10])
            ...
            for k in range(np.size(self.GroupNames)):
                ...
                for i in np.unique(self.GroupNbs[k]):
                    temp_Voc.append(self.performance_data[np.logical_and(
                        self.nbs[:, 0] == i, self.IVselected), 1])
                    temp_Jsc.append(self.performance_data[np.logical_and(
                        self.nbs[:, 0] == i, self.IVselected), 2])
                    temp_FF.append(self.performance_data[np.logical_and(
                        self.nbs[:, 0] == i, self.IVselected), 3])
                    temp_PCE.append(self.performance_data[np.logical_and(
                        self.nbs[:, 0] == i, self.IVselected), 4])

                    if self.isDual == True:
                        # Reverse sweep
                        temp_Voc_rev.append(self.performance_data[np.logical_and(self.nbs[:,
                                                                                          0] == i, self.IVselected), 7])
                        temp_Jsc_rev.append(self.performance_data[np.logical_and(self.nbs[:,
                                                                                          0] == i, self.IVselected), 8])
                        temp_FF_rev.append(self.performance_data[np.logical_and(self.nbs[:,
                                                                                         0] == i, self.IVselected), 9])
                        temp_PCE_rev.append(self.performance_data[np.logical_and(self.nbs[:,
                                                                                          0] == i, self.IVselected), 10])
                ...
                # reverse
                if self.isDual == True:
                    Voc_dev_rev.append(np.asarray(
                        temp_Voc_rev, dtype="float64"))
                    Jsc_dev_rev.append(np.asarray(
                        temp_Jsc_rev, dtype="float64"))
                    FF_dev_rev.append(np.asarray(temp_FF_rev, dtype="float64"))
                    PCE_dev_rev.append(np.asarray(
                        temp_PCE_rev, dtype="float64"))

            labels_ = self.GroupNames.astype('U13')

        else:
            warnings.warn(
                "Please enter groups or devices in the sortParams function and nothing else.", category=UserWarning)
    ...
    def plotOverview(self, Voc_dev, Jsc_dev, FF_dev, PCE_dev, groupOrDevice, forwardOrReverse, labels_, avg):
        ...
        if avg == True:
            ...
            ylimVoc = (np.min(avgVoc) - np.max(stdVoc) - 0.01,
                       np.max(avgVoc) + np.max(stdVoc) + 0.01)
            ylimJsc = (np.min(avgJsc) - np.max(stdJsc) - 0.2,
                       np.max(avgJsc) + np.max(stdJsc) + 0.2)
            ylimFF = (np.min(avgFF) - np.max(stdFF) - 0.5,
                      np.max(avgFF) + np.max(stdFF) + 0.5)
            ylimPCE = (np.min(avgPCE) - np.max(stdPCE) - 0.2,
                       np.max(avgPCE) + np.max(stdPCE) + 0.2)
            xylabel = ["", "Voc (V)"]
            tickInterval = 1
    ...
    def showHeroDevices(self, groupOrDevice):
        ...
        if groupOrDevice == "group" and np.size(self.GroupNames) == 0:
            warnings.warn(
                "Use the assign group dialog before plotting the Hero Devices of each Group!", category=UserWarning)
            return
        ...
        if groupOrDevice == "group":
            for groupNb in range(np.size(self.GroupNames)):
                ...
                print(self.GroupNames.astype('U13')[groupNb] + "'s hero pixel is " +
                      str(self.nbs[np.where(self.performance_data[:, 4] ==
                                            np.max(self.performance_data[np.where(np.in1d(self.nbs[:, 0],
                                                                                          self.GroupNbs[groupNb][:]))][:, 4]))[0][0], :]))
        ...
        # Plot hero pixel of every device
            ...
            for devNb in np.unique(self.nbs[:, 0]):
                ...
                dataToPlot.append(self.IVdata[2 * np.where(self.performance_data[:, 4] ==
                                                           np.max(self.performance_data[np.where(self.nbs[:, 0] == devNb)][:,
                                                                                                                           4]))[0][0]])
                dataToPlot.append(self.IVdata[2 * np.where(self.performance_data[:, 4] ==
                                                           np.max(self.performance_data[np.where(self.nbs[:, 0] == devNb)][:,
                                                                                                                           4]))[0][0] + 1])
                labels = np.append(labels, self.IVlabels[np.where(self.performance_data[:, 4]
                                                                  == np.max(self.performance_data[np.where(self.nbs[:, 0] == devNb)][:,
                                                                                                                                     4]))[0][0]])
    ...
    def plot_intDepIV(self):
        ...
        for perfNb in range(np.shape(file_data_array)[0]):
            try:
                regex = re.compile(r'\d+')
                extracted_nbs = [int(x) for x in regex.findall(
                    file_data_array[perfNb, 0].rsplit('/', 1)[-1])]
                # print(extracted_nbs)
                ...
                if np.any(file_data_array[perfNb, :] == 0) or np.any(file_data_array[perfNb, :] == float("inf")):
                    file_data_array[perfNb,
                                    file_data_array[perfNb, :] == float("inf")] = 0
        ...
        for elem2Del in range(np.shape(infZeroElems)[0]):
            elems_to_delete = np.append(elems_to_delete, np.where(np.logical_and(nbs_temp[:, 1] ==
                                                                                 nbs_temp[infZeroElems[elem2Del], 1], nbs_temp[:, 2] ==
                                                                                 nbs_temp[infZeroElems[elem2Del], 2], nbs_temp[:, 3] ==
                                                                                 nbs_temp[infZeroElems[elem2Del], 3])))
        # Really delete the unlogical pixels and all belonging measurements
        file_data_array = np.delete(
            file_data_array, np.unique(elems_to_delete), axis=0)
        ...
        for k in range(np.size(self.GroupNames)):
            ...
            for m in np.unique(nbs_temp[:, 0]):
                ...
                if np.size(temp_Voc) > 0:
                    ...
                    if np.size(temp_Voc) == 1:
                        ...
                        store_plot_err = np.append(
                            store_plot_err, np.std(temp_Voc))
                    elif np.size(temp_Voc) == 0:
                        print(i)
                        warnings.warn(
                            "Not possible to plot this intensity dependent IV", category=UserWarning)
                    else:
            ...
            if np.size(store_plot) > 0:
                ...
                try:
                    popt, pcov = curve_fit(
                        lambda t, a, b: a * np.log(t) + b, x, store_plot, maxfev=10000)
                    ...
                    warnings.warn("Fit for " + str(self.GroupNames.astype('U13')
                                                   [k]) + " could not be done for some reason", category=UserWarning)

        self._ax.set_xscale("log")
        self._ax.autoscale(enable=True, tight=True)
        self._ax.set_ylim(self._ax.get_ylim()[
                          0], self._ax.get_ylim()[1] * 1.02)
        self._ax.set_xlim(self._ax.get_xlim()[
                          0] * 0.8, self._ax.get_xlim()[1] * 1.2)
        self._ax.grid(True)
        self._ax.set_ylabel("V$_{OC}$ (V)", fontsize=self.fonts_)
        self._ax.set_xlabel("Light Intensity (mW/cm$^2$)",
                            fontsize=self.fonts_)
        ...
        self._ax.tick_params(width=self.ax_ticks_width,
                             length=self.tick_length)
        self._ax.tick_params(which="minor", direction="in", bottom=True,
                             top=True, length=self.tick_length/4*3, width=self.ax_ticks_width)
        self._ax.tick_params(which="major", direction="in",
                             bottom=True, top=True, width=self.ax_ticks_width)
        self._ax.tick_params(which="major", direction="in",
                             left=True, right=True, width=self.ax_ticks_width)
        ...
        leg = self._ax.legend(handles, labels, frameon=False,
                              fontsize=self.fonts_, loc=self.legend_pos)
        leg.get_frame().set_alpha(0.4)
        leg.set_draggable(True)  # Make it draggable
    ...
    def load_EQE(self):
        ...
        # Do plotting and formatting
        ylim = (0, 100)
        # Name of columns in files to read in
        names = ["wavelength", "device_current", "incident_light_int", "EQE"]
        xylabel = ["Wavelength (nm)", "EQE (%)"]
        tickInterval = 100
        plotVars = ["wavelength", "EQE"]
        dataToPlot, labels = self.readData(files, names, plotVars)
        self._ax = self.fig.figure.subplots()
        self.plot(dataToPlot, self.EQElabels, xylabel,
                  tickInterval, ylim_=ylim, ax_=self._ax)
    ...
    def load_UVVIS(self):
        ...
        try:
            ...
            warnings.warn(
                "UV-VIS data can't be plotted. Most probably there is no valid UV-VIS data.", category=UserWarning)
    ...
    def plot_UVVisNorm(self):
        ...
        try:
            if self.isnorm == False:
                ...
                dataNorm = np.empty(
                    (int(dataToPlot.shape[0]), xDataLims.shape[1]))
            ...
            warnings.warn(
                "Some error happened. Normalization is not possible.", category=UserWarning)
    ...
    def fit_UVVis_dialog(self):
        ...
        Title = QtWidgets.QLabel(
            "Enter the limits for the two fits of the onset")
    ...
    def fit_UVVis(self):
        ...
        try:
            fit1_boundaries = (int(self.fit1LowerLim.text()),
                               int(self.fit1UpperLim.text()))
            fit2_boundaries = (int(self.fit2LowerLim.text()),
                               int(self.fit2UpperLim.text()))
            fit_plot_sub = 60
        except:
            warnings.warn("Please enter fitting boundaries",
                          category=UserWarning)
        ...
        for linenb in range(len(self._ax.lines)):
            ...
            if self._ax.lines[linenb].get_visible():
                ...
                band_gap_wl = self.calcEg(
                    wavelength, absorption, fit1_boundaries, fit2_boundaries)
    ...
    def fitlin(self, l, dat, ran):
        ...
        popt, pcov = curve_fit(lambda t, a, b: np.power(
            10, (a + b * t)),  x,  y, p0=[0.01, 0.01], maxfev=10000)
    ...
    def calcEg(self, l, A, st_range, bs_range):
        ...
        slope_f1, intercept_f1 = self.fitlin(
            np.array(l), np.array(A), st_range)
        slope_f2, intercept_f2 = self.fitlin(
            np.array(l), np.array(A), bs_range)
    ...
    def TPV_dialog(self):
        ...
        self.plot(self.TPVdata, self.TPVlabels, xylabel, tickInterval,
                  ax_=self._ax, xlim_=xlim, yticksoff_=True)
    ...
    def TPC_dialog(self):
        ...
        # Turn around the array (for standard architecture)
        self.plot(self.TPCdata, self.TPClabels, xylabel, tickInterval,
                  ax_=self._ax, xlim_=xlim, yticksoff_=True)
    ...
    def plot_USR(self):
        ...
        # try:
        filepath = str(
            QtWidgets.QFileDialog.getExistingDirectory(self, "Open File"))
    ...
    def load_TPVTPC(self):
        ...
        tickInterval = 0.5
        self._ax = self.fig.figure.subplots()
        self.plot(dataToPlot, labels, xylabel, tickInterval,
                  ax_=self._ax, xlim_=xlim, yticksoff_=True)
    ...
    def mobilityMeasurement(self):
        ...
        for nb_files in range(len(filesDIV)):
            ...
            # pixel_nbDIV = extracted_nbsDIV[2]
            regex = re.compile(r'\d+')
            pixel_nbDIV = [int(x) for x in regex.findall(
                filesDIV[nb_files].rsplit('/', 1)[-1].rsplit('_', 5)[3])][0]
            ...
            nbsDIV_temp = np.append(
                nbsDIV_temp, [[dev_nb, pixel_nbDIV, int(scan_nbDIV)]], axis=0)
            DIVlabels_temp = np.append(DIVlabels_temp, "d" + str(dev_nb) + "p" +
                                       str(pixel_nbDIV) + "s" + str(scan_nbDIV))
    ...
    def extractMobility(self):
            ...
            for k in self.GroupNbs[m]:
                for i in range(np.shape(self.nbsDIV)[0]):
                    if self.IVselected[i] == True and k == self.nbsDIV[i, 0]:
                        self.slopeMobility[i], self.x_new[i], self.y_new[i], self.x_min[i], self.x_max[i] = self.fitMobility(
                            self.DIVdata[(2*i):(2*i+2)])
        ...
        for i in range(np.size(mobility_group)):
            print("Group " + str(self.GroupNames[i]) + ": " +
                  str(np.round(mobility_group[i], 8)) + " cm2/Vs")
    ...
    def fitMobility(self, dat):
        ...
        try:
            ...
            popt, pcov = curve_fit(
                lambda t, a, b, c: a * (t - b) ** 2 + c, xdata, ydata, maxfev=10000)
            ...
            warnings.warn(
                "Could not fit the curve probably it is nowhere quadratic", category=UserWarning)
            return 0, 0, 0, 0, 0
    ...
    def load_ELQE(self):
        ...
        filesELQE = np.array(
            [f for f in glob.glob(filepath + "*_LIVdata.txt")])
